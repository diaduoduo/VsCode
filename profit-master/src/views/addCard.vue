<template>
    <div id="addCard">
        <top-tab title="银行卡"></top-tab>
        <div class="form-group container">
            <label for="1">
                <div class="form-item">
                    <div class="li-title">卡号</div>
                    <div class="form-input">
                        <input type="text" id="1" placeholder="请输入银行卡号码" class="input" v-model="info.CardNumber" @blur="checkBank" :key="CardNumber">
                    </div>
                </div>
            </label>
            <label for="2">
                <div class="form-item">
                    ...
                    <div class="form-input">
                        <input type="text" id="2" placeholder="请选择发卡银行" class="input"  v-model="info.Bank" :key="Bank">
                        <!--<img src="../assets/img/icon_righ@2x.png" alt="" class="arrow">-->
                    </div>
                </div>
            </label>
            <label for="3">
                <div class="form-item">
                    <div class="li-title">支行</div>
                    <div class="form-input">
                        <input type="text" id="3" placeholder="请选择发卡支行银行" class="input"  v-model="info.OpeningBank" :key="OpeningBank">
                    </div>
                </div>
            </label>
            <label for="4">
                <div class="form-item">
                    <div class="li-title">持卡人</div>
                    <div class="form-input">
                        <input type="text" id="4" placeholder="请输入持卡人姓名" class="input"  v-model="info.Name" :key="Name">
                    </div>
                </div>
            </label>
            <label for="5">
                <div class="form-item">
                    ...
                    <div class="form-input">
                        <input type="text" id="5" placeholder="请输入持卡人身份证号" class="input"  v-model="info.IDNumber" :key="IDNumber">
                    </div>
                </div>
            </label>
            <label for="6">
                <div class="form-item">
                    <div class="li-title">联系方式</div>
                    <div class="form-input">
                        <input type="text" id="6" placeholder="请输入持卡人手机号" class="input"  v-model="info.Mobile" :key="Mobile">
                    </div>
                </div>
            </label>
        </div>
...
<script>
    export default {
        ...
        methods:{
            checkBank:function(){
                if(!(/\d{15}|\d{19}/.test(this.info.CardNumber.replace(/\s/g, "")))){
                    this.$toast("请输入正确的银行卡号");
                    return;
                }
                this.info.Bank=bankCardAttribution(this.info.CardNumber).bankName;
            },
            save:function () {
                if(!this.info.CardNumber.replace(/\s/g, "")){
                    this.$toast("请输入银行卡号");
                    return;
                }
                if(!(/\d{15}|\d{19}/.test(this.info.CardNumber.replace(/\s/g, "")))){
                ...
                if(!this.info.Bank.replace(/\s/g, "")){
                    this.$toast("请输入发卡银行");
                    return;
                }
                if(!this.info.OpeningBank.replace(/\s/g, "")){
                    this.$toast("请输入发卡支行银行");
                    return;
                }
                ...
                if(!this.info.IDNumber.replace(/\s/g, "")){
                    this.$toast("请输入持卡人身份证号");
                    return;
                }
                if(!(/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(this.info.IDNumber.replace(/\s/g, "")))){
                    this.$toast("请输入正确的持卡人身份证号");
                    return;
                }
                if(!this.info.Mobile){
                ...
                if(!(/^((13[0-9])|(17[0-1,6-8])|(15[^4,\\D])|(18[0-9]))\d{8}$/.test(this.info.Mobile.replace(/\s/g, "")))){
                    this.$toast("请输入正确的手机号");
                    return;
                }
                this.$post("/api/RevenueRecords/SaveBankCard",{
                    "ID":this.info.ID,
                    "Name": this.info.Name.replace(/\s/g, ""),
                    "IDNumber": this.info.IDNumber.replace(/\s/g, ""),
                    "Bank": this.info.Bank.replace(/\s/g, ""),
                    "CardNumber": this.info.CardNumber.replace(/\s/g, ""),
                    "Mobile": this.info.Mobile.replace(/\s/g, ""),
                    "OpeningBank": this.info.OpeningBank.replace(/\s/g, "")
                }).then(res=>{
                    if (res.ReturnCode === 0) {
                        this.$router.go(-1);
                    } else {
        ...
        created:function(){
            let card=window.sessionStorage.getItem("card");
            if(card){
